        ...
        <div class="text-center scroll">
            <table id="table_status" class="table table-bordered text-center style_table">
                ...
                <tr>
                    ...
                    <th>Request Status</th>
                    <th>Descriptions</th>
                    <th>Name of the deliverer</th>
                    <th>Delivery receipt print</th>
                    <th>Show details</th>
                    <th>Issuance of Rayvarz remittance</th>
                </tr>
                </thead>
                ...
                <tr v-for="(request,index) in request_all" :key="index">
                    ...
                    <td label="Name of the deliverer">
                        <label v-if="request.request_detail[0].delivery!=''">{{request.request_detail[0].delivery[0].user.first_name}} {{request.request_detail[0].delivery[0].user.last_name}}</label>
                    </td>
                    ...
                    <td label="Issuance of Rayvarz remittance">
                        <button type="button" class="btn btn-success" data-toggle="modal" @click="beforeCreateHavale(request)">Remittance
                        </button>
                    </td>
                </tr>
                </tbody>
...
export default {
    ...
    data() {
        ...
        loadRequest() {
            axios.get('api/requests/getAllArchiveStatus').then(({data}) => (this.request_all = data.data))
        },
        request_info(index) {
           ...
           this.clearForm()
            Swal.fire({
                ...
                if (response.isConfirmed) {
                    for (let i=0;i<request.request_detail.length;i++)
                    {
                        this.item_array.push({
                            "fiscalYear": moment(new Date()).format("jYYYY"),
                            "warehouseId": request.request_detail[i].warehouse.rayvarz_id,
                            "itemDataId": request.request_detail[i].product.itemDataId,
                            "centerId": request.request_detail[i].center.rayvarz_id,
                            "qty":'-'+ request.request_detail[i].amount,
